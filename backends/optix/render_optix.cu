...
__device__ void unpack_material(const MaterialParams &p, float2 uv, DisneyMaterial &mat)
{
    ...
    if (IS_TEXTURED_PARAM(mask)) {
        const uint32_t tex_id = GET_TEXTURE_ID(mask);
        mat.base_color =
            make_float3(tex2D<float4>(launch_params.textures[tex_id], uv.x, uv.y));
    } else {
...
__device__ float3 sample_direct_light(const DisneyMaterial &mat,
                                      const float3 &hit_p,
                                      const float3 &n,
                                      const float3 &v_x,
                                      const float3 &v_y,
                                      const float3 &w_o,
                                      const QuadLight *lights,
...
extern "C" __global__ void __raygen__perspective_camera()
{
    ...
    for (uint32_t s = 0; s < launch_params.samples_per_pixel; ++s) {
        ...
        do {
            ...
            illum = illum + path_throughput * sample_direct_light(mat,
                                                                  hit_p,
                                                                  v_z,
                                                                  v_x,
                                                                  v_y,
