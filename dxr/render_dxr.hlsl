...
#include "lights.hlsl"
#include "torrance_sparrow_brdf.hlsl"
#include "util.hlsl"
...
float textured_scalar_param(const float x, in const float2 uv)
{
    ...
    if (IS_TEXTURED_PARAM(mask)) {
        ...
        return textures[NonUniformResourceIndex(tex_id)].SampleLevel(
            tex_sampler, uv, 0)[channel];
    }
    return x;
}
...
void unpack_material(inout DisneyMaterial mat, uint id, float2 uv)
{
    ...
    if (IS_TEXTURED_PARAM(mask)) {
        const uint32_t tex_id = GET_TEXTURE_ID(mask);
        mat.base_color =
            textures[NonUniformResourceIndex(tex_id)].SampleLevel(tex_sampler, uv, 0).xyz;
    } else {
...
// Lazy debugging to swap out BRDFs easily
inline float3 current_brdf(in const DisneyMaterial mat,
...
float3 sample_direct_light(in const DisneyMaterial mat,
                           in const float3 hit_p,
                           in const Basis basis,
                           in const float3 w_o,
                           inout uint ray_count,
                           inout LCGRand rng)
{
    float3 illum = 0.f;
    ...
    // Sample the light to compute an incident light ray to this point
    {
        ...
        float bsdf_pdf = current_pdf(mat, basis, w_o, light_dir);
        ...
        if (light_pdf >= EPSILON && bsdf_pdf >= EPSILON && shadow_hit.hit == 0) {
            float3 bsdf = current_brdf(mat, basis, w_o, light_dir);
            float w = power_heuristic(1.f, light_pdf, 1.f, bsdf_pdf);
            illum = bsdf * light.emission.rgb * abs(dot(light_dir, basis.z)) * w / light_pdf;
        }
    }
    ...
    // Sample the BRDF to compute a light sample as well
    {
        float3 w_i;
        float bsdf_pdf;
        float3 bsdf = sample_current_brdf(mat, basis, w_o, rng, w_i, bsdf_pdf);
...
#endif
                if (shadow_hit.hit == 0) {
                    illum += bsdf * light.emission.rgb * abs(dot(w_i, basis.z)) * w / bsdf_pdf;
                }
            }
        }
    }
...
[shader("raygeneration")] void RayGen() {
    ...
    do {
        ...
        TraceRay(
            scene, RAY_FLAG_FORCE_OPAQUE, 0xff, PRIMARY_RAY, 1, PRIMARY_RAY, ray, payload);
#ifdef REPORT_RAY_STATS
        ++ray_count;
#endif
        ...
        // but want to treat it as the front face?
        ...
        float3 w_i;
        float pdf;
        float3 bsdf = sample_current_brdf(mat, shading_basis, w_o, rng, w_i, pdf);
        if (pdf == 0.f || all(bsdf == 0.f)) {
            break;
        }
        path_throughput *= bsdf * abs(dot(w_i, shading_basis.z)) / pdf;
    ...
    const float4 accum_color =
        (float4(illum, 1.0) + frame_id * accum_buffer[pixel]) / (frame_id + 1);
...
[shader("miss")] void ShadowMiss(inout OcclusionHitInfo occlusion
                                 : SV_RayPayload) { occlusion.hit = 0; }
...
[shader("closesthit")] void ClosestHit(inout HitInfo payload, Attributes attrib) {
    ...
    if (num_uvs > 0) {
        ...
        uv = (1.f - attrib.bary.x - attrib.bary.y) * uva + attrib.bary.x * uvb +
             attrib.bary.y * uvc;
    }

    payload.color_dist = float4(uv, 0, RayTCurrent());
    float3x3 inv_transp =
        float3x3(WorldToObject4x3()[0], WorldToObject4x3()[1], WorldToObject4x3()[2]);
