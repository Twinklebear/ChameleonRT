...
const std::array<float, 9> triangle_verts = {0.0, -0.5, 0.0, 0.5, 0.5, 0.0, -0.5, 0.5, 0.0};
...
uint32_t get_memory_type_index(uint32_t type_filter,
...
int main(int argc, const char **argv)
{
    ...
    SDL_Window *window = SDL_CreateWindow(
        "SDL2 + Vulkan", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, win_width, win_height, 0);
    ...
    VkPhysicalDevice vk_physical_device = VK_NULL_HANDLE;
    {
        ...
        const bool has_discrete_gpu =
            std::find_if(devices.begin(), devices.end(), [](const VkPhysicalDevice &d) {
    ...
    uint32_t graphics_queue_index = -1;
    {
        ...
        for (uint32_t i = 0; i < num_queue_families; ++i) {
            // We want present and graphics on the same queue (kind of assume this will be supported
            // on any discrete GPU)
    ...
    // Setup swapchain, assume a real GPU so don't bother querying the capabilities, just get what
    // we want
    ...
    std::vector<VkImageView> swapchain_image_views;
    {
        ...
        vkGetSwapchainImagesKHR(
            vk_device, vk_swapchain, &num_swapchain_imgs, swapchain_images.data());
    ...
    VkDeviceMemory vertex_mem = VK_NULL_HANDLE;
    {
        ...
        alloc_info.memoryTypeIndex = get_memory_type_index(
            mem_reqs.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, mem_props);
        ...
        alloc_info.memoryTypeIndex = get_memory_type_index(
            mem_reqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, mem_props);
        ...
        vkMapMemory(
            vk_device, upload_mem, 0, info.size, 0, reinterpret_cast<void **>(&upload_mapping));
    ...
    uint64_t blas_handle = 0;
    {
        ...
        // TODO WILL: For a single triangle it requests 64k output and 64k scratch? It seems like a
        // lot.
        ...
        alloc_info.memoryTypeIndex =
            get_memory_type_index(mem_reqs.memoryRequirements.memoryTypeBits,
        ...
        vkCmdBuildAccelerationStructure(command_buffer,
                                        &accel_info,
                                        VK_NULL_HANDLE,
                                        0,
                                        false,
        ...
        CHECK_VULKAN(
            vkGetAccelerationStructureHandle(vk_device, blas, sizeof(uint64_t), &blas_handle));
    ...
    VkDeviceMemory instance_mem = VK_NULL_HANDLE;
    {
        ...
        upload_mapping->flags = VK_GEOMETRY_INSTANCE_FORCE_OPAQUE_BIT_NV |
    ...
    uint64_t tlas_handle = 0;
    {
        ...
        CHECK_VULKAN(
            vkGetAccelerationStructureHandle(vk_device, tlas, sizeof(uint64_t), &tlas_handle));
    ...
    VkDescriptorSetLayout desc_layout = VK_NULL_HANDLE;
    {
        ...
        const std::array<VkDescriptorSetLayoutBinding, 2> desc_set = {tlas_binding, fb_binding};
        ...
        CHECK_VULKAN(
            vkCreateDescriptorSetLayout(vk_device, &desc_create_info, nullptr, &desc_layout));
        ...
        CHECK_VULKAN(
            vkCreatePipelineLayout(vk_device, &pipeline_create_info, nullptr, &pipeline_layout));
        ...
        CHECK_VULKAN(
            vkCreateShaderModule(vk_device, &create_info, nullptr, &closest_hit_shader_module));

        std::array<VkPipelineShaderStageCreateInfo, 3> shader_create_info = {
        ...
        std::array<VkRayTracingShaderGroupCreateInfoNV, 3> rt_shader_groups = {
        ...
        CHECK_VULKAN(vkCreateRayTracingPipelines(
            vk_device, VK_NULL_HANDLE, 1, &rt_pipeline_create_info, nullptr, &rt_pipeline));
    }
    ...
    VkDeviceMemory sbt_mem = VK_NULL_HANDLE;
    {
        ...
        CHECK_VULKAN(vkGetRayTracingShaderGroupHandles(
            vk_device, rt_pipeline, 0, 3, shader_identifiers.size(), shader_identifiers.data()));
    ...
    // Finally, record the rendering commands
    {
        for (size_t i = 0; i < command_buffers.size(); ++i) {
            ...
            vkCmdBindPipeline(
                command_buffers[i], VK_PIPELINE_BIND_POINT_RAY_TRACING_NV, rt_pipeline);
            ...
            vkCmdTraceRays(command_buffers[i],
                           ...
                           raytracing_props.shaderGroupHandleSize,
                           sbt_buffer,
                           raytracing_props.shaderGroupHandleSize * 2,
                           raytracing_props.shaderGroupHandleSize,
                           VK_NULL_HANDLE,
                           0,
                           0,
    ...
    while (!done) {
        ...
        CHECK_VULKAN(vkAcquireNextImageKHR(vk_device,
                                           vk_swapchain,
        ...
        CHECK_VULKAN(
            vkWaitForFences(vk_device, 1, &vk_fence, true, std::numeric_limits<uint64_t>::max()));
    }
