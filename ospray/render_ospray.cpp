...
void RenderOSPRay::initialize(const int fb_width, const int fb_height)
{
    ...
    fb = ospNewFrameBuffer(
        osp::vec2i{fb_width, fb_height}, OSP_FB_SRGBA, OSP_FB_COLOR | OSP_FB_ACCUM);
    img.resize(fb_width * fb_height);
}

void RenderOSPRay::set_mesh(const std::vector<float> &verts, const std::vector<uint32_t> &indices)
{
